...
# bcryptjs is required by seed-admin.js at runtime but is NOT included in the
# Next.js standalone bundle (the app inlines it into server chunks). Copy it
# explicitly from the builder's full node_modules so the admin-seed script can
# resolve `require('bcryptjs')` inside the container.
