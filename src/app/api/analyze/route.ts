...
export async function POST(req: Request) {
    ...
    try {
        ...
        logger.info({ requiresImage: analysisResult.requiresImage, subject: subjectChinese }, '📊 Image requirement check');

        // 配图已由 AI 在同一次识图调用里以 <figure_svg> 返回（SVG 代码），
        // 不再单独调用图片生成接口（免费额度不支持，且多一次往返）。
        logger.info({
            requiresImage: analysisResult.requiresImage,
            hasFigureSvg: !!(analysisResult as any).figureSvg,
            figureSvgLength: (analysisResult as any).figureSvg?.length || 0,
        }, '🖼️ Figure SVG check');
