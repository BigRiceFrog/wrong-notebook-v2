        ...
        try {
            const errorItem = await prisma.errorItem.create({
                data: {
                    ...
                    originalImageUrl,
                    // 带图标记：AI 识别返回 requiresImage（题目含图形/配图）时置 true；
                    // 兜底：即使 requiresImage 漏判，只要带了 figureSvg（说明确实生成了配图）也算带图题。
                    // 带图题在举一反三/打印时直接展示原图，不依赖 AI 文字/变式
                    hasImage: (requiresImage ?? false) || !!extractFigureSvg(figureSvg),
                    questionText,
                    answerText,
                    analysis,
                    ...
                    // 配图 SVG 必须过安全校验才落库：客户端传来的内容一律不可信；
                    // 且仅当 requiresImage=true 时才保留——无图题严禁配图（与 AI provider 的服务端守卫一致）
                    figureSvg: requiresImage ? extractFigureSvg(figureSvg) : null,
                    masteryLevel: 0,
                    tags: {
