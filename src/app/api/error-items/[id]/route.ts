...
import { extractFigureSvg } from "@/lib/ai/figure-svg";
...
export async function PUT(
    ...
    try {
        ...
        const { knowledgePoints, gradeSemester, paperLevel, questionText, answerText, analysis, subjectId, wrongAnswerText, mistakeAnalysis, mistakeStatus, geogebraCommands, cardImageUrl, geometryData, figureSvg } = body;
        ...
        if (geometryData !== undefined) updateData.geometryData = geometryData || null;
        if (figureSvg !== undefined) {
            const svg = extractFigureSvg(figureSvg);
            updateData.figureSvg = svg;
            // 有配图 SVG 说明题确实带图，同步 hasImage 标记（保证举一反三/打印走原图展示）
            if (svg) updateData.hasImage = true;
        }
