...
import { generateCardPng } from "@/lib/card-image";
...
export default function AddErrorPage() {
    ...
    useEffect(() => {
        let cancelled = false;
        if (step === "review" && parsedData && cardRef.current) {
            const t = setTimeout(() => {
                generateCardPng(cardRef.current as HTMLElement)
                    .then((url) => { if (!cancelled) setCardImage(url); })
                    .catch((e) => console.warn("生成卡片图失败", e));
            }, 600);
