            ...
            <div className="container mx-auto p-4 space-y-6 pb-20">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        ) : (!item.hasImage && <MarkdownRenderer content={item.questionText} />)}

                        {/* 带图题直接展示原图（下方"查看原题图片"默认展开），不再使用 AI 重绘的 SVG */}
                        {!isEditingQuestion && item.originalImageUrl && (
                            <img
                        ...
                        {item.cardImageUrl && (
                            <div className="pt-3">
                                <div className="mt-4 pt-3 border-t">
                                    <h4 className="text-sm font-semibold text-primary mb-2">📋 题目卡片（可打印）</h4>
                                    <img src={item.cardImageUrl} alt="题目卡片" className="w-full rounded-lg border shadow-sm" />
                                    <p className="text-xs text-muted-foreground mt-1 text-center">右键 → 打印 / 另存为图片</p>
                                </div>
                            </div>
                        )}
                    </CardContent>
