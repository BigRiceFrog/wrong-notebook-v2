...
# Sync database schema to match Prisma schema (db push, not migrate deploy).
# This project uses `prisma db push` workflow — new columns (e.g. cardImageUrl,
# figureSvg) are added via db push and may not have corresponding migration files.
# Using `migrate deploy` here would silently skip those columns on old volumes,
# causing P2022 "column does not exist" errors after image upgrades.
echo "[Entrypoint] Syncing database schema with prisma db push..."
cd /app && $PRISMA_BIN db push --schema=./prisma/schema.prisma --accept-data-loss && {
    echo "[Entrypoint] Schema sync completed successfully."
} || echo "[Entrypoint] Schema sync failed (non-fatal, continuing...)."
