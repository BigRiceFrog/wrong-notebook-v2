...
docker run -d --name wrong-notebook \
  ...
  ghcr.io/bigricefrog/wrong-notebook-v2
```
...
1.  **下载配置文件**：
    ```bash
    curl -o docker-compose.yml https://raw.githubusercontent.com/bigricefrog/wrong-notebook-v2/refs/heads/main/docker-compose.yml
    ```
2.  **启动服务**：
    ```bash
    docker-compose up -d
...
```bash
git clone https://github.com/bigricefrog/wrong-notebook-v2.git
cd wrong-notebook
```
...
默认管理员账户：
- **登录名**: `admin`（不带邮箱，直接输入 `admin` 即可）
- **密码**: `123456`（可通过 Docker 环境变量 `PASSWORD` 覆盖；不设置则保持默认）
